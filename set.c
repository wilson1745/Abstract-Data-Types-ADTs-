...
set* set_init(void)	{
   set *p;
   p = (set*)calloc(1, sizeof(set));		/* Making a set for p */
	
   if (p == NULL) {
      ON_ERROR("Creation of Array Failed\n");
   }
   p->ua = (arr*)calloc(1, sizeof(arr));	/* Making a arr space in a set */
   if(p == NULL) {
      ON_ERROR("Creation of Array Failed\n");
   } 
   p->ua->data = (arrtype*)calloc(ARRDEFSZ, sizeof(arrtype));	/* Making arrays */
   if(p->ua->data == NULL) {
      ON_ERROR("Creation of Array Failed\n");
   }
   p->ua->pz = ARRDEFSZ;					
   p->sz = 0;					/* Start with size 0 */					
...
set* set_copy(set* s) {
   ...
   p = set_init();
   for(i = 0;i < s->sz;i++) {		  /* for(i=0;i<s->ua->pz;i++) */ 
...
void set_insert(set* s, arrtype l) {
   if(s == NULL) {			/*Using free(&s) if s=set_init();*/
      return ;
   }
   if(set_contains(s, l) == 0) {
      arr_set(s->ua, s->sz, l);		/*Or using s->ua->data[s->sz] = l;*/
      s->sz++;
   }
}
...
void set_remove(set* s, arrtype l) {
   ...
   for(i = 0 ; i < s->sz ; i++) {			/* Preparing to delete the target value once you find it */
      if(l == s->ua->data[i]) {
         for(j = i ; j < s->sz ; j++) {
            if(j == s->sz-1) {
               s->ua->data[j] = 0;
               ...
               s->ua->data[j] = s->ua->data[j+1];		/* Making the next value to replace the last value */
            }
         }
      s->sz = s->sz-1;
      }
